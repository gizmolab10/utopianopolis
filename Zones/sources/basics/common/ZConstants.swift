...
let        kWidthToHeightRatio = CGFloat( 0.75)
...
let    kDeprecatedLineOfDashes = kHyphen.repeatedFor(25)
...
let          kHalfLineOfDashes = kSuperHyphen.repeatedFor(6)
let           kDefaultFontSize = kDefaultBaseFontSize + kFontDelta
let              kLineOfDashes = kHalfLineOfDashes + kSuperHyphen + kHalfLineOfDashes
...
let              kSearchFilter = "search filter"
...
let            kCKRepositoryID = "cloudkit identifier"
...
let        kDefaultRecordNames = kRootName + kColon + kRootName + kColon + kFavoritesRootName
...
let      kMigrationTestCDStore = "test3"
...
let               kDoubleColon = kColon + kColon
let            kCrumbSeparator = "      "
...
let               kScratchZone = Zone.uniqueZoneNamed(kScratchRootName, recordName: kScratchRootName, databaseID: .mineID, checkCDStore: true)

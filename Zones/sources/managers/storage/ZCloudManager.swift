...
//        }
    ...
    func queryFor(_ recordType: String, with predicate: NSPredicate, properties: [String], batchSize: Int = kBatchSize, onCompletion: RecordErrorClosure?) {
    ...
    func queryWith(_ predicate: NSPredicate, batchSize: Int = kBatchSize, onCompletion: RecordErrorClosure?) {
    ...
    func search(for searchString: String, onCompletion: ObjectClosure?) {
        ...
        queryWith(predicate) { (iRecord, iError) in
    ...
    func fetchLost(_ onCompletion: IntClosure?) {
        ...
        self.queryWith(predicate, batchSize: kMaxBatchSize) { (iRecord, iError) in
            if  let ckRecord      = iRecord {
...
//                                        onCompletion?(0)
    ...
    func fetch(for type: String, properties: [String], since: Date, before: Date?, _ onCompletion: RecordsErrorClosure?) {
        let predicate = self.predicate(since: since, before: before)
        var retrieved = [CKRecord] ()

        queryFor(type, with: predicate, properties: properties) { (iRecord, iError) in
            if iError != nil {
                onCompletion?([], iError!)
    ...
    func fetchNew(_ onCompletion: IntClosure?) {
        ...
        // get T = lastSyncDate
        // fetch since T
        // if error, fetch split by two

        fetch(for: kZoneType, properties: Zone.cloudProperties(), since: lastSyncDate, before: nil) { (iCKRecords, iError) in
            if iError != nil {
                self.bam("aaaaaaaghpppffflt!")
            } else {
                FOREGROUND {
                    if iCKRecords.count == 0 {
                        onCompletion?(0)
    ...
    func xfetchParents(_ onCompletion: IntClosure?) {
    ...
    func xfetchChildren(_ onCompletion: IntClosure?) {
        ...
        if count > 0 {
            ...
            queryWith(predicate, batchSize: kMaxBatchSize) { (iRecord, iError) in
                if  let ckRecord = iRecord {
                    if !retrieved.contains(ckRecord) {
                        retrieved.append(ckRecord)
                    }
    ...
    func xfetchTraits(_ onCompletion: IntClosure?) {
    ...
    func xfetchBookmarks(_ onCompletion: IntClosure?) {

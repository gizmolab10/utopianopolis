...
class ZRecord: NSObject {
    ...
    var isAutoGenerated: Bool {
        get {
            return _isAutoGenerated || (record?.recordID.isAutoGenerated ?? true)    // true because lack of record prevents save to db
        }

        set {
            _isAutoGenerated = newValue
        }
    }
    ...
    func requireFetch() { addState(.doNotSave) }
    func allowSave()    { removeState(.doNotSave)}
    ...
    func maybeNeedSave() {
        if !isAutoGenerated, !needsDestroy, !needsFetch, !doNotSave {
            removeState(.needsMerge)
            addState   (.needsSave)
        }
    }
    ...
    func maybeNeedMerge() {
        if !isAutoGenerated, alreadyExists, !doNotSave, !needsSave, !needsMerge, !needsDestroy {
            addState(.needsMerge)
        }
    }
    ...
    func maybeNeedFetch() {
        if !isAutoGenerated, !alreadyExists {
            addState(.needsFetch)
        }
    }
